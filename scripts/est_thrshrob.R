...
irftvarrob_chol <- lapply(order, function(oo){
  run_tvarrob <- btvar(Yraw = Yraw1[,oo], plag = plag, d.min = 1, d.max = 4, Zraw = Zraw1, nsave = draws, nburn = burnin, thin = thin, 
  ...
  for(irep in 1:thindraws){
    for(hh in 1:h){
      temp    <- gen_compMat(A=run_tvarrob$store$A_store[irep,,,hh], M=K, p=plag)
      compMat <- temp$Cm
      Jm      <- temp$Jm
      SIGMA   <- run_tvarrob$store$L_store[irep,,,hh]%*%diag(exp(run_tvarrob$store$Diags_store[irep,,hh]))%*%t(run_tvarrob$store$L_store[irep,,,hh])
    ...
    return(apply(irftvarrob_chol_store[,,"BAAT10",,], c(2,3,4), quantile, c(.05,.10,.16,.50,.84,.90,0.95), na.rm = TRUE))
})
