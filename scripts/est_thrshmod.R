...
irfvar_chol_store <- array(NA_real_, c(thindraws, M, M, nhor, h),
                           dimnames=list(NULL, colnames(Yraw1), colnames(Yraw1), seq(nhor), c("regime 1", "regime 2")))
irfvar_extInstr_store <- array(NA_real_, c(thindraws, M, M, nhor, h),
                               dimnames=list(NULL, colnames(Yraw1), colnames(Yraw1), seq(nhor), c("regime 1", "regime 2")))
irfvar_extInstr_store_old <- array(NA_real_, c(thindraws, M, M, nhor, h),
                                   dimnames=list(NULL, colnames(Yraw1), colnames(Yraw1), seq(nhor), c("regime 1", "regime 2")))
for(irep in 1:thindraws){
  for(hh in 1:h){
    ...
    irfvar_chol_store[irep,,,,hh] <- impresp1
    ...
    irfvar_extInstr_store[irep,,,,hh] <- impresp2
    ...
    irfvar_extInstr_store_old[irep,,,,hh] <- impresp3
  }
}

irfvar_chol <- apply(irfvar_chol_store[,,"BAAT10",,], c(2,3,4), quantile, c(.05,.10,.16,.50,.84,.90,0.95), na.rm = TRUE)
irfvar_ext  <- apply(irfvar_extInstr_store[,,"BAAT10",,], c(2,3,4), quantile, c(.05,.10,.16,.50,.84,.90,.95), na.rm = TRUE)
irfvar_ext2 <- apply(irfvar_extInstr_store_old[,,"BAAT10",,], c(2,3,4), quantile, c(.05,.10,.16,.50,.84,.90,.95), na.rm = TRUE)
