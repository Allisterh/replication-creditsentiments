...
run_tvarext <- btvar(Yraw = cbind(Yraw1,factors[,1:q]), plag = plag, d.min = 1, d.max = 4, Zraw = Zraw1, nsave = draws, nburn = burnin, thin = thin, 
...
for(irep in 1:thindraws){
  for(hh in 1:h){
    temp    <- gen_compMat(A=run_tvarext$store$A_store[irep,,,hh], M=K, p=plag)
    compMat <- temp$Cm
    Jm      <- temp$Jm
    SIGMA   <- run_tvarext$store$L_store[irep,,,hh]%*%diag(exp(run_tvarext$store$Diags_store[irep,,hh]))%*%t(run_tvarext$store$L_store[irep,,,hh])
    ...
    res <- run_tvarext$store$res_store[irep,,]
...
irftvarext_chol <- apply(irftvarext_chol_store[,,"BAAT10",,], c(2,3,4), quantile, c(.05,.10,.16,.50,.84,.90,0.95), na.rm = TRUE)
irftvarext_ext  <- apply(irftvarext_ext_store[,,"BAAT10",,], c(2,3,4), quantile, c(.05,.10,.16,.50,.84,.90,.95), na.rm = TRUE)

#------ Convergence Diagnostics

A <- run_tvarext$store$A_store
D <- run_tvarext$store$Diags_store
L <- run_tvarext$store$L_store
Aprior <- run_tvarext$store$Aprior_store
Lprior <- run_tvarext$store$Lprior_store
lambda2 <- run_tvarext$store$lambda2_store
tau <- run_tvarext$store$tau_store
...
for(hh in 1:h){
  for(kk in 1:(K*plag+1)){
    for(ii in 1:K){
      ...
      Ineff_A[kk,ii,hh] <- thindraws/effectiveSize(temp)
...
Ineff_D <- array(NA_real_, c(K, h))
raftd_D <- array(NA_real_, c(K, h))
gewek_D <- array(NA_real_, c(K, h))
for(hh in 1:h){
  for(kk in 1:K){
    temp <- as.mcmc(D[,kk,hh])
    Ineff_D[kk,hh] <- thindraws/effectiveSize(temp)
    raftd_D[kk,hh] <- raftery.diag(temp,r=0.015)$resmatrix[,"I"]
    gewek_D[kk,hh] <- geweke.diag(temp, frac1=0.1, frac2=0.5)$z
  }
}

Ineff_L <- array(NA_real_, c(K, K, hh))
raftd_L <- array(NA_real_, c(K, K, hh))
gewek_L <- array(NA_real_, c(K, K, hh))
for(hh in 1:h){
  for(kk in 2:K){
    for(ii in 1:(kk-1)){
...
for(hh in 1:h){
  for(kk in 1:(K*plag)){
    for(ii in 1:K){
      ...
      Ineff_Aprior[kk,ii,hh] <- thindraws/effectiveSize(temp)
...
Ineff_Lprior <- array(NA_real_, c(K, K, h))
raftd_Lprior <- array(NA_real_, c(K, K, h))
gewek_Lprior <- array(NA_real_, c(K, K, h))
for(hh in 1:h){
  for(kk in 2:K){
    for(ii in 1:(kk-1)){
      temp <- as.mcmc(Lprior[,kk,ii,hh])
      Ineff_Lprior[kk,ii,hh] <- thindraws/effectiveSize(temp)
      raftd_Lprior[kk,ii,hh] <- raftery.diag(temp,r=0.015)$resmatrix[,"I"]
      gewek_Lprior[kk,ii,hh] <- geweke.diag(temp, frac1=0.1, frac2=0.5)$z
    }
  }
}
...
tvarext_conv_reg1 = list(Ineff=mean(c(Ineff_A[,,1],Ineff_D[,1],Ineff_L[,,1],Ineff_Aprior[,,1],Ineff_Lprior[,,1],Ineff_lambda2[,1],Ineff_tau[,1]),na.rm=TRUE),
...
tvarext_conv_reg2 = list(Ineff=mean(c(Ineff_A[,,2],Ineff_D[,2],Ineff_L[,,2],Ineff_Aprior[,,2],Ineff_Lprior[,,2],Ineff_lambda2[,2],Ineff_tau[,2]),na.rm=TRUE),
...
rm(Yraw1, Qraw1, fit.res, ihor, impact, impresp1, impresp2, irep, irftvarext_chol_store, irftvarext_ext_store, 
   ...
   Sig12, Sig21, Sig22, SIGMA, temp, ZZp, mm, hh, Ineff_A, Ineff_D, Ineff_L, Ineff_Aprior, Ineff_Lprior, Ineff_lambda2, 
